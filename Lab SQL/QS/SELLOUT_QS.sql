...
insert into per.MAESTRO_PROMOTOR
(PromotorID, PromotorIDClie, DescPromotor, GrpID)
SELECT
NEXT VALUE FOR [per].[seq_promotorid] OVER(ORDER BY A.PromotorIDClie),
A.PromotorIDClie,
A.PromotorIDClie,
A.GrpID
	FROM (
		SELECT DISTINCT A.PromotorIDClie,A.GrpID
			FROM mkt.sellout_stage A
			LEFT JOIN per.MAESTRO_PROMOTOR B
				ON A.PromotorIDClie=B.PromotorIDClie
		WHERE A.PromotorIDClie IS NOT NULL AND B.PromotorID IS NULL
	) A;
...
INSERT INTO per.GLP_SELLOUT (Fecha, GrpID, CadenaIDClie, OficinaIDClie, GrupoTratIDClie, VendedorIDClie, SupervisorIDClie,PromotorIDClie, ProIDClie, PdvIDClie, UnidDesp, MontoDespCliente)
SELECT
		Fecha,
		GrpID,
		CadenaIDClie,

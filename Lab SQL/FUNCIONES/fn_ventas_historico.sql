USE genomma;
GO
IF OBJECT_ID('vw.fn_ventas_historico','IF') IS NOT NULL
    DROP FUNCTION vw.fn_ventas_historico;
GO
CREATE FUNCTION vw.fn_ventas_historico(@grupoid VARCHAR(3),@semanas int)
    RETURNS TABLE
  AS
  RETURN (
SELECT

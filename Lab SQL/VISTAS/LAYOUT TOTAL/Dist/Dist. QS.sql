...
CREATE VIEW vw.[DistQS.01 Informacion]
AS
SELECT
...
CREATE VIEW vw.[DistQS.02 Cuenta]
AS
SELECT
[GrpID] GrupoID,
[GrpNombre],
...
CREATE VIEW vw.[DistQS.03 Centro]
AS
SELECT
[GrpID] GrupoID,
CadenaID,
...
CREATE VIEW vw.[DistQS.04 Geografia]
AS
SELECT
  UbigeoID,
  Pais,
...
CREATE VIEW vw.[DistQS.05 Punto de Venta]
AS
SELECT
  PdvID,
  PdvIDClie,
...
CREATE VIEW vw.[DistQS.06 Producto]
AS
SELECT
A.[ProPstID],
A.[ProPstCodBarras],
...
DROP VIEW vw.[DistQS.07 Tiempo];
CREATE VIEW vw.[DistQS.07 Tiempo]
AS
SELECT
...
WHERE Fecha >=(
  SELECT min(Fecha) FROM per.GLP_SELLOUT WHERE GrpID in  ('307')
) and Fecha <=(
  SELECT max(Fecha) FROM per.GLP_SELLOUT WHERE GrpID in  ('307')
);

DROP VIEW vw.[DistQS.08 TiempoMes];
CREATE VIEW vw.[DistQS.08 TiempoMes] AS
SELECT
  [Año-Mes],
  Año,
  Mes,
...
FROM (
  ...
  FROM (
         SELECT
           concat(A.Año,'-',RIGHT('00'+ CAST(A.mes AS VARCHAR(2)), 2)) [Año-Mes],
           A.Año,
           A.Mes,
           A.[Año.Mes],
           MIN(Fecha)                            Fecha_Inicio,
           MAX(Fecha)                            Fecha_Fin
         FROM vw.[DistQS.07 Tiempo] A
         GROUP BY CONVERT(VARCHAR, A.Año * 100 + A.Mes), A.Año, A.Mes, A.[Año.Mes]
       ) A
) A;

CREATE VIEW vw.[DistQS.09 Oficina]
AS
SELECT
GrpID GrupoID,
[OficinaID],
...
CREATE VIEW vw.[DistQS.10 Tratamiento Comercial]
AS
SELECT
GrpID GrupoID,
...
CREATE VIEW vw.[DistQS.11 Supervisor]
  AS
SELECT
  GrpID GrupoID,
  SupervisorID,
...
CREATE VIEW vw.[DistQS.12 Promotor]
  AS
SELECT
  GrpID GrupoID,
  PromotorID,
...
CREATE VIEW vw.[DistQS.13 Vendedor]
AS
SELECT
GrpID GrupoID,
[VendedorID],

--SELECT * into mkt.stock_dg_day FROM mkt.STOCK_DIGA_HIST

DELETE FROM MKT.STOCK_DIGA_HISTW
WHERE FECHA in
(SELECT DISTINCT convert(varchar,DATEADD(day,-1,CONVERT(DATE,SUBSTRING(A.FECHA,7,4)+SUBSTRING(A.FECHA,4,2)+ SUBSTRING(A.FECHA,1,2),112)),112) FROM mkt.stock_dg_day A);

INSERT INTO mkt.STOCK_DIGA_HISTW
SELECT
...
cast(NULL as varchar(6)) [PdvIDClie],

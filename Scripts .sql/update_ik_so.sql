...
WHERE [Fecha] in
(SELECT DISTINCT CONCAT(SUBSTRING([PERIODO],1,4),SUBSTRING(PERIODO,6,2),SUBSTRING(PERIODO,9,2)) FROM mkt.sellout_ik_day A)
;

INSERT INTO mkt.SELLOUT_IK_HISTW
SELECT CONCAT(SUBSTRING(A.PERIODO,1,4),SUBSTRING(A.PERIODO,6,2),SUBSTRING(A.PERIODO,9,2)) Fecha
      ,A.COD_PRODUCTO
      ,A.COD_PRODUCTO_PROVEEDOR
      ,A.DESCRIPCION
      ,A.MARCA
      ,A.ESTADO_PROD
      ,A.UMB
      ,A.COD_LOCAL
      ,A.COD_LOCAL_PROVEEDOR
      ,A.DESCRIPCION_LOCAL
      ,A.ESTADO_LOCAL
      ,A.FORMATO
      ,A.TIPO
...
cast(A.VTA_PERIODO_UNID as decimal(18,6)) [UnidDesp],
cast(A.COSTO_DE_VENTA_PERIODO_S as decimal(18,6)) [MontoDespCliente]
FROM (
      SELECT CONCAT(SUBSTRING(A.PERIODO,1,4),SUBSTRING(A.PERIODO,6,2),SUBSTRING(A.PERIODO,9,2)) Fecha
        ,A.COD_PRODUCTO
        ,A.COD_PRODUCTO_PROVEEDOR

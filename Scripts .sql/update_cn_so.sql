DELETE FROM MKT.SELLOUT_CONTI_HISTW
WHERE FECHA in
      (SELECT DISTINCT SUBSTRING(A.FECHA,1,4)+SUBSTRING(A.FECHA,6,2)+ SUBSTRING(A.FECHA,9,2) FECHA FROM mkt.sellout_cn_day A);

INSERT INTO mkt.SELLOUT_CONTI_HISTW
SELECT
  SUBSTRING(A.FECHA,1,4)+SUBSTRING(A.FECHA,6,2)+ SUBSTRING(A.FECHA,9,2) FECHA,
  PRODUCTO,
  UN,
  DESC_PROD,
  CODIGO,
...
cast(COALESCE(CONVERT(VARCHAR,TRY_CONVERT(INT,A.GIRO)),A.GIRO) as varchar(4)) [GrupoTratIDClie],
cast(COALESCE(CONVERT(VARCHAR,TRY_CONVERT(BIGINT,A.VENDEDOR)),A.VENDEDOR) as varchar(7)) [VendedorIDClie],
cast(NULL as varchar(8)) [SupervisorIDClie],
cast(COALESCE(CONVERT(VARCHAR,TRY_CONVERT(BIGINT,A.PRODUCTO)),A.PRODUCTO)  as varchar(12)) [ProIDClie],
cast(COALESCE(CONVERT(VARCHAR,TRY_CONVERT(BIGINT,A.CODIGO)),A.CODIGO)  as varchar(8)) [PdvIDClie],

...
def get_path(tiempo,path,file):
    return file.format(path, str(tiempo[u'Año_GL']), str(tiempo[u'Semana_GL']).zfill(2),str(tiempo[u'Año']), str(tiempo[u'Semana_GL']).zfill(2))
...
file_so='{0}{1}\\Semana {2}\\{3}_{4}_Tottus_SellOut_c.xlsm'
file_st='{0}{1}\\Semana {2}\\{3}_{4}_Tottus_Inventario.xlsx'
path= 'Z:\Proyecto visores\Data Información Colombia\Peru\DATA Cliente\Tottus\\'
...
if os.path.exists(unicode(path_so, "utf-8")) or 1==1:
    #modificando ktr
    ktr= dir_ktr + r'sell_out_tottus.ktr'
    replace_ktr(ktr,path_so)
    ktr= dir_ktr + r'stock_tottus.ktr'
    replace_ktr(ktr,path_st)
    #modificando query
    #path_sql=r'D:\Python lab\update_ik_so.sql'
    #replace_date(path_sql,str(tiempo[u'Fecha']))
    #path_sql=r'D:\Python lab\update_ik_st.sql'
    #replace_date(path_sql,str(tiempo[u'Fecha']))

    #ejecutando job
    #call(dir_bat + r'sellout_tt_kjb.bat')
    #call(dir_bat + r'stock_tt_kjb.bat')
else:

...
file_so='{0}{1}\\Semana {2}\\{3}_{4}_{5}_Digalimenta_SellOut.txt'
file_st='{0}{1}\\Semana {2}\\{3}_{4}_{5}_Digalimenta_Inventario.txt'
path= 'Z:\Proyecto visores\Data Información Colombia\Peru\DATA Cliente\Digalimenta\\'
...
if os.path.exists(unicode(path_so, "utf-8"))and os.path.exists(unicode(path_st, "utf-8")):
    #modificando ktr
    ktr= dir_ktr + r'sell_out_digalimenta.ktr'
    replace_ktr(ktr,path_so)
    ktr= dir_ktr + r'stock_digalimenta.ktr'
    ...
    #ejecutando job
    call(dir_bat + r'sellout_dg_kjb.bat')
    call(dir_bat + r'stock_dg_kjb.bat')
else:

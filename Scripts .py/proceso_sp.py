...
file_so='{0}{1}\\Semana {2}\\{3}_{4}_{5}_SPP_SellOut.csv'
file_st='{0}{1}\\Semana {2}\\{3}_{4}_{5}_SPP_Inventario.csv'
path= 'Z:\Proyecto visores\Data Información Colombia\Peru\DATA Cliente\Supermercados Peruanos\\'
...
if os.path.exists(unicode(path_so, "utf-8"))and os.path.exists(unicode(path_st, "utf-8")):
    #modificando ktr
    ktr= dir_ktr + r'sell_out_spp.ktr'
    replace_ktr(ktr,path_so)
    ktr= dir_ktr + r'stock_spp.ktr'
    replace_ktr(ktr,path_st)

    #ejecutando job
    call(dir_bat + r'sellout_sp_kjb.bat')
    call(dir_bat + r'stock_sp_kjb.bat')
else:

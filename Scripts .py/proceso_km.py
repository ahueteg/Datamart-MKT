...
file_so='{0}{1}\\Semana {2}\\{3}_{4}_{5}_Keymark_SellOut.txt'
file_st='{0}{1}\\Semana {2}\\{3}_{4}_{5}_Keymark_Inventario.txt'
path= 'Z:\Proyecto visores\Data Información Colombia\Peru\DATA Cliente\KeyMark\\'
...
if os.path.exists(unicode(path_so, "utf-8")):
    #modificando ktr
    ktr= dir_ktr + r'sell_out_keymark.ktr'
    replace_ktr(ktr,path_so)
    ktr= dir_ktr + r'stock_keymark.ktr'
    ...
    #ejecutando job
    call(dir_bat + r'sellout_km_kjb.bat')
    call(dir_bat + r'stock_km_kjb.bat')
else:
